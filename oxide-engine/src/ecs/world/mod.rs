...
mod tests {
  ...
  fn spawn_query_multi_component_multi_bundle() {
    ...
    world
      .spawn((Position(15, 6), Velocity(2, 1), 0_u32))
      .unwrap();
    world
      .spawn((Position(25, 6), Velocity(3, 1), "asdfsaf", 1_f32))
      .unwrap();
  ...
  fn spawn_query_nested_multi_component_bundle() {
    ...
    world.spawn(Position(1, 2)).unwrap();
    world
      .spawn((Position(25, 6), Velocity(3, 1), "asdfsaf", 1_u32))
      .unwrap();
    world
      .spawn((Position(15, 6), Velocity(30, 1), "alfsaf", 2_f32))
      .unwrap();
    world
      .spawn((Position(35, 6), Velocity(3, 1), "aaf", 3_f64))
      .unwrap();

    let mut query = world
      .query::<(((&[Position], &[Velocity]), &[&'static str]), &[u32])>()
      .unwrap();
    let (_, pos) = query.next().unwrap();
    assert_eq!(
      pos,
      (((&Position(25, 6), &Velocity(3, 1)), &"asdfsaf"), &1_u32)
    );
  }
