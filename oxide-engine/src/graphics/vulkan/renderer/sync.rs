...
impl VulkanRendererSync {
    ...
    pub fn wait_nth(&self, device: &Device, current_frame: usize) -> Result<(), VulkanError> {
        Ok(self.command_buffer_fences[current_frame].wait(device)?)
    }

    pub fn reset_nth(&self, device: &Device, current_frame: usize) -> Result<(), VulkanError> {
        Ok(self.command_buffer_fences[current_frame].reset(device)?)
    }
