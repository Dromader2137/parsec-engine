...
impl MaterialData {
    pub fn bind(&self, resources: &ResourceCollection, command_buffer: Arc<CommandBuffer>, camera_id: u32, transform_id: u32) {
        ...
        for (set_index, set) in self.descriptor_sets.iter().enumerate() {
            ...
            command_buffer.bind_descriptor_set(descriptor_set.clone(), self.base.pipeline.clone(), set_index as u32);
        }
    }
}
